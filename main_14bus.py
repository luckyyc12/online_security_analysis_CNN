...
import numpy as np
from utils_14bus import *
from ops_14bus import *

# Data Dimensions
img_h = img_w = 14  # 28  # MNIST images are 28x28
img_size_flat = img_h * img_w  # 28x28=784, the total number of pixels
n_classes = 16  # 10  # Number of classes, one class per digit
n_channels = 1
...
num_filters1 = 2  # 16  # There are 16 of these filters.
...
h1 = 7*7*2  # 128  # Number of neurons in fully-connected layer.
...
fc1 = fc_layer(layer_flat, h1, 'FC1', use_relu=True)
output_logits = fc_layer(fc1, n_classes, 'OUT', use_relu=False)
final_tensor = tf.nn.sigmoid(output_logits, name='final_tensor')
...
with tf.variable_scope('Train'):
    ...
    with tf.variable_scope('Accuracy'):
        ...
        correct_prediction = tf.equal(tf.round(final_tensor), y, name='correct_pred')
        correct_prediction = tf.count_nonzero(correct_prediction, axis=1)/n_classes
